...
0. Начни с [урока 0](lessons/00-podgotovka.md): там клонирование, настройка и что делать, если всё сломал.
1. Проходи уроки по порядку, это папка [`lessons/`](lessons/). **Одного чтения мало:** в конце каждого урока
   есть блок **🛠 Сделай сам**. В нём объяснено, зачем это нужно на работе, что делать руками и как проверить себя.
2. После урока посмотри, как то же самое выглядело «вживую» в этом репо (таблица ниже).
3. Если забыл команду, открывай [cheatsheet.md](cheatsheet.md).
4. В конце проверь себя по [вопросам с собеседований](SOBES.md).
